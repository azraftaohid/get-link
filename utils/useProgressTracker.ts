...
export const useProgressTracker = <T>(initKeys: T[]): UseProgressTracker<T> => {
	...
	return {
		...
		hasCompleted: useRef((t: T) => mapping.current.get(t) === "failed").current,
		hasCancelled: useRef((t: T) => mapping.current.get(t) === "cancelled").current,
		hasFailed: useRef((t: T) => mapping.current.get(t) === "failed").current,
		completedCount,
		failedCount,
		cancelledCount,
	};
...
export interface UseProgressTracker<T> {
	keys: T[],
	setKeys: Dispatch<SetStateAction<T[]>>,
	removeKey: (t: T) => void,
	markCompleted: (t: T) => void,
	markCancelled: (t: T) => void,
	markFailed: (t: T) => void,
	hasCompleted: (t: T) => boolean,
	hasCancelled: (t: T) => boolean,
	hasFailed: (t: T) => boolean,
